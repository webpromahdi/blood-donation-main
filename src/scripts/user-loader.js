...
function updateHeaderUserInfo(user) {
    ...
    for (const selector of nameSelectors) {
        ...
        if (el && (el.textContent.includes('User') || el.textContent.includes('Smith') || el.textContent.includes('Hospital') || el.textContent.includes('Guest'))) {
    ...
    if (roleEl) {
        ...
        if (currentText.includes('administrator') || currentText.includes('donor') ||
            currentText.includes('hospital') || currentText.includes('seeker') ||
            currentText.includes('staff') || currentText.includes('portal')) {
    ...
    welcomeEls.forEach(el => {
        if (el.textContent.includes('Welcome back, John')) {
            el.textContent = el.textContent.replace('John', user.name?.split(' ')[0] || 'User');
        }
    });
}
